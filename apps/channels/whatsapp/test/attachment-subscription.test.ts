...
// no longer touches attachment events at all, so the two readers of the
// same session event stream can never both call `deliverAttachment` for
// the same event.
...
function newBridge(): { bridge: WhatsAppBridge; calls: Array<[string, Record<string, unknown>]> } {
  ...
  // paths under test route through this spy instead of the real Baileys API.
...
test('an in-turn attachment is delivered exactly once, not doubled, with both the per-turn loop and the persistent subscription reading the same stream', async () => {
  ...
  await withFetch(
    ...
    async () => {
      ...
      }).waitForAgentResponse('sess-2', '15551230002@s.whatsapp.net');
